<x-default-layout>
            ...
            <article
                ...
                <header class="mb-4 lg:mb-6 not-format">
                    ...
                    <div class="flex justify-between items-center">
                        <p class="capitalize">{{ $post->getReadingTime() }} min read</p>
                        @if ($post->created_at != $post->updated_at)
                            <small title="This post has been edited.">
                                Edited
                            </small>
                        @endif
                    </div>
    ...
    @if ($same_author_posts->count() > 0)
        <aside aria-label="More from author" class="py-8 lg:py-24 bg-gray-50 dark:bg-gray-800">
            <div class="px-4 mx-auto max-w-screen-xl">
                <h2 class="mb-8 text-2xl font-bold text-gray-900 dark:text-white">More from {{ $post->author->name }}
                </h2>
                <div class="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">

                    @foreach ($same_author_posts as $post)
                        <article class="max-w-xs">
                            <a href="{{ route('posts.show', $post->slug) }}">
                                <img src="{{ $post->banner_url ?? '' }}" class="mb-5 rounded-lg"
                                    alt="{{ $post->title }}">
                            </a>
                            <h2 class="mb-2 text-xl font-bold leading-tight text-gray-900 dark:text-white line-clamp-1">
                                <a href="{{ route('posts.show', $post->slug) }}">{{ $post->title }}</a>
                            </h2>
                            <p class="mb-4 font-light text-gray-500 dark:text-gray-400 line-clamp-2">
                                {{ $post->excerpt }}
                            </p>
                            <a href="{{ route('posts.show', $post->slug) }}"
                                class="inline-flex items-center font-medium underline underline-offset-4 text-primary-600 dark:text-primary-500 hover:no-underline">
                                Read in {{ $post->getReadingTime() }} minutes
                            </a>
                        </article>
                    @endforeach
    ...
    @if ($related_posts->count() > 0)
        <aside aria-label="Related Posts" class="py-8 lg:py-24 bg-gray-50 dark:bg-gray-800">
            <div class="px-4 mx-auto max-w-screen-xl">
                <h2 class="mb-8 text-2xl font-bold text-gray-900 dark:text-white">Related Posts
                </h2>
                <div class="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    @foreach ($related_posts as $post)
                        <article class="max-w-xs">
                            ...
                            <a href="{{ route('posts.show', $post->slug) }}"
                                class="inline-flex items-center font-medium underline underline-offset-4 text-primary-600 dark:text-primary-500 hover:no-underline">
                                Read in {{ App\Models\Post::calculateReadingTime($post->content) }} minutes
                            </a>
                        </article>
                    @endforeach
